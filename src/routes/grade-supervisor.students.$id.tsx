    ...
    <div dir="rtl" className="font-vazir space-y-6">
      ...
      <Card className="p-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
          <MiniStat label="مجموع دوز" value={`${toFa(bio.totalMinutes)} دق`} tone="bg-indigo-50 text-indigo-700" />
          <MiniStat label="تعداد دوز" value={toFa(bio.doseCount)} tone="bg-violet-50 text-violet-700" />
          <MiniStat
            label="آخرین چکاب"
            value={bio.latest ? `${toFa(bio.latest.score)}٪` : "—"}
            tone={bio.latest && bio.latest.score < 60 ? "bg-rose-50 text-rose-700" : "bg-emerald-50 text-emerald-700"}
          />
          <MiniStat
            label="میانگین چکاب"
        ...
        <div className="space-y-2">
          {BIO_MICROS.map((m) => {
            ...
            return (
              <div key={m.id} className="flex items-center justify-between text-xs gap-3 px-3 py-2 rounded-xl border border-slate-100">
                <span className="text-slate-700 truncate">{m.title}</span>
                <span className="flex items-center gap-2 shrink-0">
                  <span className="text-slate-500">دوز {toFa(minutes)} دق</span>
                  <span className={avg == null ? "text-slate-400" : avg < 60 ? "text-rose-600 font-bold" : "text-emerald-600 font-bold"}>
                    {avg == null ? "—" : `${toFa(avg)}٪`}
                  </span>
                </span>
              </div>
