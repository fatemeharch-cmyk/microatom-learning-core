...
import { calendarEvents, type CalendarEvent } from "@/lib/parent-mock";
...
const kindMap: Record<CalendarEvent["kind"], { label: string; cls: string }> = {
  class: { label: "کلاس", cls: "bg-secondary text-secondary-foreground border-border" },
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <CalendarRange className="h-6 w-6 text-primary" /> تقویم آموزشی فرزند
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            نمای یکپارچه از کلاس‌ها، آزمون‌ها، جلسات و رویدادها
          </p>
        </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between gap-3 space-y-0">
          <CardTitle className="text-base">
            {cursor.toLocaleDateString("fa-IR", { year: "numeric", month: "long" })}
          </CardTitle>
