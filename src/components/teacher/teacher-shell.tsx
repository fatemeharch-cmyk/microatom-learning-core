...
import { ArrowLeft, Bell, GraduationCap, Languages } from "lucide-react";
                ...
                <SidebarGroupContent>
                  <SidebarMenu>
                    {items.map((item) => {
                      const active =
                        pathname === item.url ||
                        (item.url !== "/teacher" && pathname.startsWith(item.url));
                      return (
                ...
                <SidebarMenuItem>
                  <SidebarMenuButton asChild>
                    <Link to="/" className="flex items-center gap-2">
                      <ArrowLeft
                        className={`h-4 w-4 shrink-0 ${dir === "rtl" ? "rotate-180" : ""}`}
                      />
